...
//   Training data file must have the following format (see building1.txt from the same folder):
//   Note that building1 data were taken from PROBEN1
...
void allocate_training_data(double **&data, double *&target, int num_training_data, int num_variables)
{
	...
	data = new double*[num_variables];
	for (int i = 0; i < num_variables; i++)
		data[i] = new double[num_training_data];
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void delete_training_data(double **&data, double *&target, int num_variables)
{
	if (data)
		for (int i = 0; i < num_variables; i++)
			delete[] data[i];
...
//---------------------------------------------------------------------------
void swap_individuals(t_mep_chromosome& dest, t_mep_chromosome& source)
{
	t_code3* tmp_c3 = dest.code;
	dest.code = source.code;
	source.code = tmp_c3;

	double* tmp_cst = dest.constants;
	dest.constants = source.constants;
	source.constants = tmp_cst;

	double f = dest.fitness;
	dest.fitness = source.fitness;
	source.fitness = f;

	int v = dest.best_index;
	dest.best_index = source.best_index;
	source.best_index = v;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void compute_eval_matrix(const t_mep_chromosome &c, int code_length, 
	int num_variables,
	...
	double **eval_matrix)
{
	...
	for (int i = 0; i < code_length; i++){   // read the chromosome from top to down
		...
		const double* a1;
		const double* a2;
		double* eval_row = eval_matrix[i];

		if (c.code[i].op < 0) {// an operator
			if ((c.code[c.code[i].addr1].op >= 0) && (c.code[c.code[i].addr1].op < num_variables)) // a variable
				a1 = training_data[c.code[c.code[i].addr1].op];
			else
				a1 = eval_matrix[c.code[i].addr1];// an operator or constant

			if ((c.code[c.code[i].addr2].op >= 0) && (c.code[c.code[i].addr2].op < num_variables)) // a variable
				a2 = training_data[c.code[c.code[i].addr2].op];
			else
				a2 = eval_matrix[c.code[i].addr2];	// an operator or constant
		}
		else {
			a1 = NULL;
			a2 = NULL;
		...
		switch (c.code[i].op) {
			...
			case  DIV_OP:  //  /
				for (int k = 0; k < num_training_data; k++)
					if (fabs(a2[k]) < 1e-6) { // test if it is too small
						is_error_case = true;
						break;
					}
			...
			default:  // a variable
				if (c.code[i].op < num_variables) {
					// do nothing because data are already kept in the training data
				}
				else
					for (int k = 0; k < num_training_data; k++)
						eval_row[k] = c.constants[c.code[i].op - num_variables];
				break;
		}
	}
}
	...
	double **eval_matrix)
{
	...
	for (int i = 0; i < code_length; i++) {   // read the chromosome from top to down
		...
		if ((c.code[i].op >= 0) && (c.code[i].op < num_variables))// a variables
			for (int k = 0; k < num_training_data; k++)
				sum_of_errors += fabs(training_data[c.code[i].op][k] - target[k]);// difference between obtained and expected
		else// an operator or constant
			for (int k = 0; k < num_training_data; k++)
...
int main(void)
{
	...
	if (!read_data("c:/Mihai/Dropbox/mep/src/console/mep/01. Symbolic regression/building1.txt", ' ', training_data, target, num_training_data, num_variables)) {
	...
	delete_training_data(training_data, target, num_variables);
